...
import {
  clearFacilitySessionToken,
  getFacilitySessionToken,
  getStoredStaffId,
  storeFacilitySessionToken,
  storeStaffId,
} from './storage';
...
const handlers = {
  async unlockFacility(facilityPin: string): Promise<void> {
    try {
      if (!facilityPin.trim()) {
        throw new Error('施設PINを入力してください。');
      }

      state.processingMessage = '施設PINを確認しています。';
      state.errorMessage = '';
      draw();

      const response = await unlockFacility(facilityPin);

      if (!response.ok) {
        throw new Error(response.error.message);
      }

      state.facilitySessionToken = response.sessionToken;
      storeFacilitySessionToken(response.sessionToken);
  ...
  selectStaff(staffId: string): void {
    state.selectedStaff =
      state.staff.find((staff) => staff.staffId === staffId) || null;

    if (state.selectedStaff) {
      storeStaffId(state.selectedStaff.staffId);
    }
  ...
  async stopRecording(): Promise<void> {
    try {
      ...
      if (!state.selectedStaff) {
        throw new Error('看護記録者が未選択です。');
      }

      state.processingMessage =
        '文字起こしと利用者別記録を作成しています。';
      draw();
  ...
  async saveRecords(): Promise<void> {
    try {
      ...
      const response = await saveRecordsApi(
        state.selectedStaff,
        state.records,
        state.facilitySessionToken,
      );
  ...
  selectCandidate(index: number, candidate: UserCandidate): void {
    state.records[index] = {
      ...
      reviewReasons: state.records[index].reviewReasons.filter(
        (reason) => !reason.includes('候補') && !reason.includes('一致'),
      ),
    };

    state.records[index].needsReview =
      state.records[index].reviewReasons.length > 0;
...
async function init(): Promise<void> {
  ...
  const sessionToken = getFacilitySessionToken();

  if (!sessionToken) {
    return;
  }

  state.facilitySessionToken = sessionToken;
  await loadMasters();
}

async function loadMasters(): Promise<void> {
  try {
    state.status = 'loadingMasters';
    state.errorMessage = '';
    draw();

    const response = await getMasters(state.facilitySessionToken);
    ...
    state.selectedStaff =
      state.staff.find((staff) => staff.staffId === storedStaffId) || null;
